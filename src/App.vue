    ...
    <section v-else class="auth-section">
      ...
      <section v-if="view === 'account' && currentDealer" class="dealer-portal">
        ...
        <header class="portal-header">
          ...
          <nav><button :class="{ active: activeTab === 'products' }" @click="openTab('products')">Ürünler</button><button :class="{ active: activeTab === 'orders' }" @click="openTab('orders')">Siparişlerim</button><button :class="{ active: activeTab === 'online-payment' }" @click="openTab('online-payment')">Online Ödeme</button></nav>
          <div class="portal-account-actions">
            <button class="portal-user" type="button" title="Firma profilini aç" @click="openTab('profile')"><span>{{ currentDealer.official.charAt(0).toUpperCase() }}</span><div><strong>{{ currentDealer.official }}</strong><small>{{ currentDealer.company }}</small></div><b>›</b></button>
            <button class="header-cart" type="button" title="Sepetimi aç" @click="openTab('payments')"><span>🛒</span><div><strong>Sepet'im</strong><small>{{ cartCount }} ürün · {{ money(cartTotal) }}</small></div><b>{{ cartCount }}</b></button>
            <button class="portal-logout" title="Çıkış yap" @click="logout">↪</button>
          </div>
        </header>
        ...
        <div v-if="activeTab === 'products'" class="search-bar"><span>⌕</span><input v-model="search" placeholder="Ürün adı ile arayın..." @input="currentPage = 1" /><div class="category-picker"><button type="button" :aria-expanded="categoryMenuOpen" @click="categoryMenuOpen = !categoryMenuOpen"><span>{{ selectedCategory }}</span><b>⌄</b></button><div v-if="categoryMenuOpen" class="category-picker-menu"><button v-for="category in categories" :key="category" type="button" :class="{ active: selectedCategory === category }" @click="selectedCategory = category; currentPage = 1; categoryMenuOpen = false">{{ category }}</button></div></div><button type="button">ARA</button></div>
        ...
        <section v-else-if="activeTab === 'orders'" class="portal-page">
          ...
          <div class="order-filters"><button class="active">Tümü <b>{{ orders.length }}</b></button><button>Hazırlanıyor <b>{{ orders.filter(order => order.status === 'Hazırlanıyor').length }}</b></button><button>Tamamlandı <b>{{ orders.filter(order => order.status === 'Tamamlandı').length }}</b></button></div>
